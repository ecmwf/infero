...
# TORCH
INFERO_CMAKE_CMD="${INFERO_CMAKE_CMD} -DENABLE_TORCH=${WITH_TORCH}"
if [ ${WITH_TORCH} == ON ]; then
  INFERO_CMAKE_CMD="${INFERO_CMAKE_CMD} -DTORCH_ROOT=${ROOT_INSTALL_DIR}"
fi

# FCKIT
INFERO_CMAKE_CMD="${INFERO_CMAKE_CMD} -DENABLE_FORTRAN=${WITH_FORTRAN}"
...
echo "Executing: ${INFERO_CMAKE_CMD} && ${INFERO_MAKE_CMD}"
